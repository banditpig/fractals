...
chooseColor colors fz z =
  (colors !!) . length . take n . takeWhile closeToZero $ fz z
  where
    n = length colors - 1
...
mapOverGrid grd fz colors =
  [[chooseColor colors fz (x :+ y) | (x, y) <- row] | row <- grd]
...
allJulia :: IO ()
allJulia = mapM_ makeSaveImage juliaConstants
  where
    makeSaveImage :: (Int, C) -> IO ()
    makeSaveImage (n, c) =
      savePngImage name $ generateImg . gridFunc grid1 . julia $ c
      where
